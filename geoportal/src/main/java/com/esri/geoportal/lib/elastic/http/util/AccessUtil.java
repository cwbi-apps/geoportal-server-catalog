...
public class AccessUtil {
  ...
  public void ensureOwner(AppUser user, String ownerField, JsonObject source) {
    if (!user.isAdmin()) {
      ...
      if (!ok)
        throw new AccessDeniedException(notOwnerMessage);
    }
  }
            ...
            if (source.containsKey(FieldNames.FIELD_SYS_ACCESS_GROUPS)) {
              ...
              if (jv != null) {
                if (jv.getValueType() == JsonValue.ValueType.STRING) {
                  ...
                  LOGGER.error("Field " + FieldNames.FIELD_SYS_ACCESS_GROUPS + " for item " + id
                      + " is not a List.");
                  throw new AccessDeniedException(accessDeniedMessage);
                }
              }
  ...
  /**
   * Get the last item accessed. return the last item
   */
