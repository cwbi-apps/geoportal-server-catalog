...
public class ReindexRequest extends BulkEditRequest {
  ...
  public AppResponse execute() throws Exception {
    ...
    /*
     * if (fromIndexName == null || fromIndexName.length() == 0) { fromIndexName = indexName; } if
     * (toIndexName == null || toIndexName.length() == 0) { toIndexName = fromIndexName; }
     */
    ...
    /*
     * long tEnd = System.currentTimeMillis(); double tSec = (tEnd - tStart) / 1000.0; String tMsg =
     * (Math.round(tSec / 60.0 * 100.0) / 100.0)+" minutes"; if (tSec <= 600) tMsg =
     * (Math.round(tSec * 100.0) / 100.0)+" seconds";
  ...
  protected void logFeedback(long tStart, long count, long total, boolean finished) {
    ...
    if (tSec <= 600)
      tMsg = (Math.round(tSec * 100.0) / 100.0) + " seconds";
   ...
   * @throws Exception
   */
  private String readXml(ElasticContext ec, ItemUtil itemUtil, String id, String indexName,
      String cue) throws Exception {
