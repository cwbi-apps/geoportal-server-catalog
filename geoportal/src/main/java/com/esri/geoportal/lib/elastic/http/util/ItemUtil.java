...
public class ItemUtil {
   ...
   * @throws Exception if an exception occurs
   */
  public JsonObject searchForFileId(String indexName, String typeName, String fileid)
      throws Exception {
    ...
    request.add("query",
        Json.createObjectBuilder().add("term", Json.createObjectBuilder().add(field, fileid)));
    ...
    int total = (hits == null || !hits.containsKey("total")) ? 0
        : hits.get("total").getValueType() == JsonValue.ValueType.NUMBER ? hits.getInt("total")
            : hits.get("total").getValueType() != JsonValue.ValueType.OBJECT ? 0
                : !hits.getJsonObject("total").containsKey("value") || hits.getJsonObject("total")
                    .get("value").getValueType() != JsonValue.ValueType.NUMBER ? 0
   ...
   * @throws Exception
   */
  public void writeItem(ElasticContext ec, MetadataDocument mdoc, String indexName)
      throws Exception {
    String contentType = "application/json;charset=utf-8";
    ElasticClient client = ElasticClient.newClient();
