...
public class BulkEditRequest extends BulkRequest {
  ...
  /*
   * 
   * url params: id=&owner=&sourceUri=&sourceRef=&taskRef= url params: query= request body:
   * {"query": ...}
   * 
   */
   ...
   * @param hit the hit
   */
  protected void appendHit(ElasticContext ec, StringBuilder data,
      com.esri.geoportal.lib.elastic.http.util.SearchHit hit) {
    ...
    if (ec.getIs6Plus())
      retryName = "retry_on_conflict";
    JsonObjectBuilder joBuilder = Json.createObjectBuilder().add("_id", hit.getId());
  ...
  protected void filterById() {
    ...
    if (values != null && values.length > 0) {
      ...
      JsonObjectBuilder joBuilder = Json.createObjectBuilder().add("values", ja);
      if (!GeoportalContext.getInstance().getElasticContext().getIs7Plus()) {
        joBuilder = joBuilder.add("type",
            GeoportalContext.getInstance().getElasticContext().getActualItemIndexType());
      }
  ...
  protected void filterByTerm(String field, String value) {
    ...
    if (value != null && value.length() > 0) {
      ...
      JsonObjectBuilder jb = Json.createObjectBuilder();
      jb.add("term", Json.createObjectBuilder().add(field, value));
      httpFilters.add(jb.build());
    }
  }
  ...
  protected String newHttpScrollerQuery(ElasticContext ec) {
    ...
    JsonArrayBuilder ja = Json.createArrayBuilder();
    for (JsonObject v : httpFilters)
      ja.add(v);
    JsonObjectBuilder jb = Json.createObjectBuilder();
    jb.add("bool", Json.createObjectBuilder().add("filter", ja));
