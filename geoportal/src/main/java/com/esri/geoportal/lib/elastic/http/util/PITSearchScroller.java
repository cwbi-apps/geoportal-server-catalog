...
public class PITSearchScroller extends BaseScroller {
  ...
  public void scroll(Consumer<SearchHit> callback) throws Exception {
    ...
    if (!getFetchSource())
      request.add("stored_fields", "_none");
    ...
    try {
      while (true) {
        ...
        if (sort.size() > 0) {
          ...
          result = client.sendPost(client.getBaseUrl() + "/_search", request.build().toString(),
              contentType);
          response = (JsonObject) JsonUtil.toJsonStructure(result);
        }
      }
      ...
      try {
        if (pitId != null) {
          ...
          client.send("DELETE", client.getBaseUrl() + "/_search/point_in_time",
              request.build().toString(), contentType);
        }
      } catch (Throwable t) {
