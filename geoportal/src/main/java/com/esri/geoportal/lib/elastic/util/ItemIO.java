...
public class ItemIO {
   ...
   * @throws Exception
   */
  public JsonObjectBuilder mixin(MetadataDocument mdoc, String elasticSource) throws Exception {
    ...
    if (hasElasticSource && src != null) {
      ...
      for (Entry<String, JsonValue> entry : src.entrySet()) {
        ...
        if (!hasEvaluatedJson) {
          bAdd = true;
        } else if (name.startsWith("_") || name.startsWith("sys_") || name.startsWith("src_")
            || name.startsWith("app_") || name.startsWith("user_")) {
          bAdd = true;
        }
        if (bAdd) {
          if (src.isNull(name))
    ...
    if (title == null || title.length() == 0)
      title = "Unknown";
    if (title != null && title.length() > 0)
      jb.add(FieldNames.FIELD_TITLE, title);
    return jb;
  }
   ...
   * @throws Exception
   */
  public IndexResponse writeItem(ElasticContext ec, MetadataDocument mdoc, String indexName,
      String xmlIndexName) throws Exception {
    ...
    IndexRequest request = new IndexRequest(indexName); // Add a document to the custom-index we
                                                        // created.
