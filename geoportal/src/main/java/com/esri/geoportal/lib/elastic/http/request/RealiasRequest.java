...
public class RealiasRequest extends BulkEditRequest {
  ...
  public AppResponse execute() throws Exception {
    ...
    /*
     * if (indexName.indexOf("_v") == -1) { throw new
     * UsageException("The indexName does not contain _v"); }
     */
              ...
              for (String k2 : jsoAliases.keySet()) {
                ...
                if (k2.equalsIgnoreCase(name)) {
                  ...
                  if (k.equalsIgnoreCase(indexName)) {
                    ...
                    actions.add(Json.createObjectBuilder().add("remove",
                        Json.createObjectBuilder().add("index", k).add("alias", k2)));
                  }
                  break;
                }
              }
    ...
    if (alreadyAliased) {
      ...
      actions.add(Json.createObjectBuilder().add("add",
          Json.createObjectBuilder().add("index", indexName).add("alias", name)));
