...
public class Kvp {
  ...
  public DeleteResponse deleteContent(ElasticContext ec, String id) throws Exception {
    // setFound(false);
    // DeleteResponse resp =
    // ec.getTransportClient().prepareDelete(getIndexName(),getIndexType(),id).get();
    // /* ES 2to5 */
  ...
  public String readHash(ElasticContext ec) throws Exception {
    ...
    if (jso != null)
      return JsonUtil.getString(jso, "hash");
    return null;
  }

  /*
   * public GetResponse readItem(ElasticContext ec) throws Exception { this.setFound(false);
   * GetRequestBuilder req = ec.getTransportClient().prepareGet(indexName,indexType,id); GetResponse
   * resp = req.get(); if (resp.isExists()) { this.setFound(true); try { Map<String,Object> source =
   * resp.getSource(); this.base64 = (String)source.get(ItemIO.FIELD_SYS_64); this.itemId =
   * (String)source.get(ItemIO.FIELD_SYS_ITEMID); this.meta =
   * (String)source.get(ItemIO.FIELD_SYS_META); } catch (Exception e) {
   * LOGGER.warn("Error reading data item, index: "+indexName+" _id: "+id,e); } } return resp; }
   */
  ...
  public JsonObject readMeta(ElasticContext ec) throws Exception {
    ...
    // ec.getTransportClient().prepareGet(getIndexName(),getIndexType(),getId());
