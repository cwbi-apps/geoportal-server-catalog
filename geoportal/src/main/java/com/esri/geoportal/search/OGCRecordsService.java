...
public class OGCRecordsService extends Application {
	...
	public Response get(@Context HttpServletRequest hsr) {
		...
		try {
			...
			response = this.generateResponse("500",
					"OGCRecords API Landing Page could not be generated.");
	...
	public Response getApi(@Context HttpServletRequest hsr) {
		...
		try {
			...
			status = Response.Status.INTERNAL_SERVER_ERROR;
			response = this.generateResponse("500",
					"OGCRecords API response could not be generated. ");
		}
		return Response.status(status).entity(response).build();
	}
	...
	public Response getConformance(@Context HttpServletRequest hsr) {
		...
		try {
			...
			responseJSON = this.generateResponse("500",
					"OGCRecords API Conformance response could not be generated.");
	...
	public Response getCollections(@Context HttpServletRequest hsr) {
		...
		try {
			...
			status = Response.Status.INTERNAL_SERVER_ERROR;
			responseJSON = this.generateResponse("500",
					"OGCRecords API collection response could not be generated.");
	...
	public Response getCollectionQueryable(@Context HttpServletRequest hsr) {
		...
		try {
			...
			status = Response.Status.INTERNAL_SERVER_ERROR;
			responseJSON = this.generateResponse("500",
					"OGCRecords API queryables response could not be generated.");
		}
		return Response.status(status).entity(responseJSON).build();
	}
	...
	public Response getCollectionMetadata(@Context HttpServletRequest hsr) {
		...
		try {
			responseJSON = this
					.readResourceFile("service/config/ogcrecords-collection-metadata.json", hsr);
	...
	public Response getItems(@Context HttpServletRequest hsr, @QueryParam("limit") int limit,
			...
			@QueryParam("title") String title, @QueryParam("provider") String provider,
		...
		try {
			...
			responseJSON = this.prepareResponse(response, hsr, bbox, limit, datetime, title,
					provider, querydsl);
			...
			status = Response.Status.INTERNAL_SERVER_ERROR;
			responseJSON = this.generateResponse("500",
					"OGCRecords API Collection metadata items response could not be generated.");
	...
	public Response getItem(@Context HttpServletRequest hsr, @PathParam("id") String id) {
		...
		try {
			...
			LOGGER.error(ESAPI.encoder().encodeForHTML(
					"Error in getting item with item id: " + id + " " + e.getMessage()));

			status = Response.Status.INTERNAL_SERVER_ERROR;
			responseJSON = this.generateResponse("500",
					"OGCRecords API Collection metadata item response could not be generated.");
	...
	private String prepareSingleItemResponse(String searchRes, HttpServletRequest hsr) {
		...
		try {
			...
			if (items != null && items.size() > 0) {
				...
				JsonObject resObj =
						(JsonObject) JsonUtil.toJsonStructure(featureContext.jsonString());
	...
	private String prepareResponse(String searchRes, HttpServletRequest hsr, String bbox, int limit,
		...
		try {
			...
			resourceFilecontext.set("$.response.numberMatched",
					numberMatched > -1 ? numberMatched : null);
			...
			resourceFilecontext.set("$.response.numberReturned",
					numberReturned > -1 ? numberReturned : null);
		...
		try {
			...
			for (String propKey : propObjKeys) {
				try {
					...
					LOGGER.trace(
							"key: " + propKey + " could not be added. Reason : " + e.getMessage());
				}
			}
	...
	private String prepareSearchQuery(Map<String, String> queryMap, String searchAfter)
		...
		if ((queryMap.containsKey("querydsl")
				&& (queryMap.get("querydsl").toString().length() > 0))) {
	...
	private String prepareBbox(String bboxString) {
		...
		String query = "";
		if (bbox.size() > 3) {
			if ((Double.parseDouble(bbox.get(0)) < -180.0)
					&& (Double.parseDouble(bbox.get(2)) >= -180.0))
				coords[0] = -180.0;
			else
				coords[0] = Double.parseDouble(bbox.get(0));
			if ((Double.parseDouble(bbox.get(1)) < -90.0)
					&& (Double.parseDouble(bbox.get(3)) >= -90.0))
				coords[1] = -90.0;
			else
				coords[1] = Double.parseDouble(bbox.get(1));
			if ((Double.parseDouble(bbox.get(2)) > 180.0)
					&& (Double.parseDouble(bbox.get(0)) <= 180.0))
				coords[2] = 180.0;
			else
				coords[2] = Double.parseDouble(bbox.get(2));
			if ((Double.parseDouble(bbox.get(3)) > 90.0)
					&& (Double.parseDouble(bbox.get(1)) <= 90.0))
				coords[3] = 90.0;
			else
		...
		if (coords.length > 3) {
			query = "{\"" + spatialType + "\": {\"" + field
					+ "\": {\"shape\": {\"type\": \"envelope\"," + "\"coordinates\": [[" + coords[0]
	...
	private String readResourceFile(String path, HttpServletRequest hsr)
