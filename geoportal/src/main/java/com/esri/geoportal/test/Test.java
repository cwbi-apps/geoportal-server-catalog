...
public class Test {
  ...
  public static void testRestHighLevelClient() throws IOException {
    ...
    try (RestHighLevelClient client = new RestHighLevelClient(builder)) {
      ...
      createIndexRequest.settings(Settings.builder() // Specify in the settings how many shards you
                                                     // want in the index.
          .put("index.number_of_shards", 4).put("index.number_of_replicas", 3));
      ...
      IndexRequest request = new IndexRequest("custom-index"); // Add a document to the custom-index
                                                               // we created.
      ...
      // Delete the document
      DeleteRequest deleteDocumentRequest = new DeleteRequest("custom-index", "1"); // Index name
                                                                                    // followed by
                                                                                    // the ID.
  ...
  public static void test1() throws Exception {
    /*
     * ElasticContext ec = GeoportalContext.getInstance().getElasticContext(); ItemIO itemIO = new
     * ItemIO(); String xml = itemIO.readXml(ec,"qqqqqqqqqqq"); System.err.println("xml\r\n"+xml);
     */
  }
  ...
  public static void testDeleteMetadata() throws Exception {
    ...
    DeleteItemRequest request = GeoportalContext.getInstance().getBean("request.DeleteItemRequest",
  ...
  public static void testNashorn() throws Exception {
    /*
     * long t1 = System.currentTimeMillis(); ScriptEngineManager engineManager = new
     * ScriptEngineManager(); ScriptEngine engine = engineManager.getEngineByName("nashorn");
     * //engine.eval("function sum(a, b) { return a + b; }");
     * //System.err.println(engine.eval("sum(1, 2);")); String item =
     * "{\"title\": \"TheTitle\",\"description\": \"The description\"}";
     * //System.err.println("--------------------------------------------------------->>>>"); String
     * path =
     * "C:/Projects/eclipse-jee-mars-1-win32-x86_64/workspace/geoportal2/src/main/resources/opensearch/item2atomEntry.js";
     * engine.eval(new FileReader(path)); Invocable invocable = (Invocable)engine; for (int
     * i=1;i<=10;i++) { String v = item.replace("TheTitle","TheTitle"+i); Object result =
     * invocable.invokeFunction("itemToAtomEntry",""+i,v); System.err.println(result);
     * //System.err.println(result.getClass()); } long t2 = System.currentTimeMillis();
  ...
  public static void testScroll2() throws Exception {
    ...
    scroller.scroll(new Consumer<SearchHit>() {
      @Override
  ...
  public static void testAlias() throws Exception {
    ...
    //
    /*
     * String pfx = name+"_v"; String idxName = null; int sfx = -1;
     * ImmutableOpenMap<String,Settings> all =
     * client.indices().prepareGetSettings("*").get().getIndexToSettings();
     * Iterator<ObjectObjectCursor<String,Settings>> iter = all.iterator(); while (iter.hasNext()) {
     * ObjectObjectCursor<String,Settings> o = iter.next(); if (o.key.startsWith(pfx)) { String s =
     * o.key.substring(pfx.length()); int i = Val.chkInt(s,-1); if (i > sfx) { sfx = i; idxName =
     * o.key; } } } if (idxName == null) { idxName = pfx+"1"; }
  ...
  public static void testKvp() throws Exception {
    ElasticContext ec = GeoportalContext.getInstance().getElasticContext();
    /*
     * String id = "88887777_xml"; String indexName = "test_v1"; DataItem dataItem = new DataItem();
     * dataItem.setIndexName(indexName); dataItem.setId(id);
     * 
     * JsonObject jso = dataItem.readMeta(ec); System.err.println(jso);
  ...
  public static void httpScroller() throws Exception {
    ...
    com.esri.geoportal.lib.elastic.http.util.Scroller scroller =
        new com.esri.geoportal.lib.elastic.http.util.Scroller();
    scroller.setIndexName("metadata");
    ...
    scroller.scroll(new Consumer<com.esri.geoportal.lib.elastic.http.util.SearchHit>() {
      @Override
