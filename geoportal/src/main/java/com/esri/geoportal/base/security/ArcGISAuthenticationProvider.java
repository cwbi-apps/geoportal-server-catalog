...
public class ArcGISAuthenticationProvider implements AuthenticationProvider {
  ...
  private List<GrantedAuthority> executeGetRoles(String username, String token, String referer)
    ...
    /*
     * String url = restBaseUrl+"/community/users/"; try { url +=
     * URLEncoder.encode(username,"UTF-8"); url +=
     * "?f=json&token="+URLEncoder.encode(token,"UTF-8"); } catch (UnsupportedEncodingException e)
     * {}
     */
    ...
    if (!responseEntity.getStatusCode().equals(HttpStatus.OK)) {
      throw new AuthenticationServiceException(
          "Error communicating with the authentication service.");
    }
    ...
    if (jso.containsKey("role") && !jso.isNull("role")) {
      ...
      if (role.equals("org_admin") || role.equals("account_admin"))
        hasOrgAdminRole = true;
      if (role.equals("org_publisher") || role.equals("account_publisher"))
        hasOrgPubRole = true;
      if (role.equals("org_user") || role.equals("account_user"))
        hasOrgUserRole = true;
    }
  ...
  private String executeGetToken(String username, String password, String referer)
    ...
    try {
      ...
      content.append("&expiration=")
          .append(URLEncoder.encode("" + getExpirationMinutes(), "UTF-8"));
  ...
  public String getRestUrl() {
    ...
    return authorizeUrl.substring(0, authorizeUrl.indexOf("/sharing/rest/oauth2/"))
        + "/sharing/rest";
  }
